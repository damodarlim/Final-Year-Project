...
include ('../includes/dbconnect.php');
  ...
  if(isset($_FILES['fileToUpload'])){
    ...
    $Destination = "upload/festival/" . $imageNewName; // Changed destination to include "activity" folder
  ...
  session_start();
  $title = mysqli_real_escape_string($conn, $_POST['festival_title']);
  $description = mysqli_real_escape_string($conn, $_POST['festdesc']);
  $addedBy = $_SESSION['member_id']; //THis will mean that who ever is logged in at that time and post. will be the author `

  $sql = "INSERT INTO festival_table (title, description, fest_img, added_by)
