...
include ('../includes/dbconnect.php');
  ...
  if(isset($_FILES['fileToUpload'])){
    ...
    $Destination = "upload/donation/". $imageNewName;
  ...
  session_start();
  $title = mysqli_real_escape_string($conn, $_POST['donation_title']);
  $amount = mysqli_real_escape_string($conn, $_POST['donation_amount']);
  $description = mysqli_real_escape_string($conn, $_POST['dondesc']);
  $addedBy = $_SESSION['member_id']; //THis will mean that who ever is logged in at that time and post. will be the author `

  $sql = "INSERT INTO donation_table (title, description, donation_img, amount, added_by)
